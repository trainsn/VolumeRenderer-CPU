...
void Trans_Stack::multpoint3d(float opoint[3], float npoint[3]) {
...
void Trans_Stack::multpoint4d(float opoint[4], float npoint[4]) {
...
void Trans_Stack::perspective(short fovy, float aspect, float near,
			      float far) {
  Matrix m;
  float *m1;
  register int i;
  ...
  angle = (double)DEGTORAD(((float)fovy / 10.0));

  m1 = &m[0][0];
  for (i=0;i<16;i++)
    *(m1+i) = 0.0;

  m[1][1] = (float) (cos(angle/2.0) / sin(angle/2.0));
  m[0][0] = m[1][1] / aspect;
  m[2][2] = -(far+near) / (far-near);
  m[2][3] = -1.0;
  m[3][2] = (float) (-(double)(2.0*far*near) / (double)(far-near));
  loadmatrix(m);
...
/* replaces the top matrix with a perspective matrix */
void Trans_Stack::window(float left, float right, float bottom, 
...
void Trans_Stack::ortho(float left, float right, float bottom,
			float top, float near, float far) {
...
void Trans_Stack::ortho2(float left, float right, float bottom, float top) {
 ...
 */
void Trans_Stack::polarview(float dist, short azim, short inc, short twist) {
 ...
 */
void Trans_Stack::lookat(float vx, float vy, float vz, float px, float py,
  ...
  m[1][1] = (float)(sqrt((double) ((px-vx)*(px-vx) + (pz-vz)*(pz-vz))) / tmp );
  m[1][2] = (float)( (double)(vy-py) / tmp );
  m[2][1] = -m[1][2];
  m[2][2] = m[1][1];
  m[3][3] = 1.0;
  multmatrix(m);
  ...
  m[0][0] = (float) ((double)(vz-pz) / tmp );
  m[0][2] = (float) -((double)(px-vx) / tmp );
  m[1][1] = 1.0;
  m[2][0] = -m[0][2];
  m[2][2] = m[0][0];
  m[3][3] = 1.0;
...
void Trans_Stack::rotate(short a, char axis) {
  ...
  angle = (double)DEGTORAD(((float)a / 10.0));
...
/* performs a rotation around an axis */
void Trans_Stack::rot(float a, char axis) {
...
void Trans_Stack::translate(float x, float y, float z) {
...
/* performs scaling */
void Trans_Stack::scale(float x, float y, float z) {
