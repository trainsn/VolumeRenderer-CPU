 		   ...
 		   image_type * imgB, enum op_code op )
{
  ...
  register float one_minus_alpha;
  int u,v; 
...
*/
	      ...
	      {
		one_minus_alpha = (float) (255 - pxlA->bp.a) / 255.0;

		pxlA->bp.r = (byte) ((float) pxlA->bp.r + 
				     (float) pxlB->bp.r*one_minus_alpha);
		pxlA->bp.g = (byte) ((float) pxlA->bp.g + 
				     (float) pxlB->bp.g*one_minus_alpha);
		pxlA->bp.b = (byte) ((float) pxlA->bp.b + 
				     (float) pxlB->bp.b*one_minus_alpha);
		pxlA->bp.a = (byte) ((float) pxlA->bp.a + 
				     (float) pxlB->bp.a*one_minus_alpha);
	      }
	  }
      else
	for(u=umin; u<=umax; u++, pxlA++, pxlB++)
...
*/
		...
		one_minus_alpha = (float) (255 - pxlB->bp.a)  / 255.0;
		
		pxlA->bp.r = (byte) ((float) pxlB->bp.r + 
				     (float) pxlA->bp.r*one_minus_alpha);
		pxlA->bp.g = (byte) ((float) pxlB->bp.g + 
				     (float) pxlA->bp.g*one_minus_alpha);
		pxlA->bp.b = (byte) ((float) pxlB->bp.b + 
				     (float) pxlA->bp.b*one_minus_alpha);
		pxlA->bp.a = (byte) ((float) pxlB->bp.a + 
				     (float) pxlA->bp.a*one_minus_alpha);
	      }
	  }
    }
