...
//  on that point
//
int volumeRender::get_value(float p[4] ,float *val, 
...
// at infinity.
//
void volumeRender::local_lighting( float *point, interpolation_state *is,
...
//
void volumeRender::local_lighting( float *n, 
		 float obj_color[4], float result[3] )
{
  uvw normal;
  float sign = 1.0;
...
{
  float p[4],q[4];
  extern  void matrix_mult(Matrix,float*,float*);

  p[0] = (float)rxmin; p[1] = (float)rymin; p[2] = (float)rzmin; p[3] = 1.0;
...
//
void volumeRender::update_bounds(float q[4]) 
{
...
{
  extern void matrix_mult(Matrix,float in[],float out[]); 
  float sumred, sumgreen, sumblue, sumalpha;
  pixel *p;
  float p1[4],p2[4],inc[4];
  float rgba[4];
  float val1;
  float outcolor[3];
  float alpha;
...
//
  ...
  s.scale((float)udim/2.0,(float)vdim/2.0,zscale);
...
float volumeRender::distance_to_viewplane(float x, float y, float z)
{
  ...
  p[0] = (float)x; p[1] = (float)y; p[2] = (float)z; p[3] = 1.0;
