...
typedef struct {
  int entries;
  float data[ITEMSPERENTRY*MAP_TABLE_SIZE];
} Map_msg;
...
//   entries
//
Map::Map(float min, float fnc1, float rgbo1[4],
...
void Map::read_file(char *name) {
  ...
  for (i=0;i<entries;i++) {
    ...
    func[i] = f;         /* depends on how float is defined */
    rvalue[i] = r;
    gvalue[i] = g;
    bvalue[i] = b;
    opacity[i] = a;
...
//
//
int Map::lookup(float value, float rgbo[4]) {
...
//
int Map::add(float value, float rgbo[4], int *pos) {
...
// adds a new range to the map map using a constant
//
void Map::add_constant(float value, float rgbo[4]) {
...
//
void Map::add_linear(float value, float rgbo[4]) {
...
// delete an entry
//
int Map::delete_entry(float value) {
...
// change an entry
//
int Map::change_entry(float value, float fnc, float rgbo[4]) {
